...
for k = 1:3
  for i=2:size(A,1)-1
    for j=2:size(A,2)-1
      if(A(i,j)==0) %se o pixel central da vizinhança de A = 1, deve ser analizado
...
figure('Name','Imagem Moedas Dilatadas')
imshow(C)

imwrite(C,'/home/gustavo/projetosGit/PDI/imMoedasDilatadas.jpg');
...
figure('Name','Imagem Moedas Erodidas')
imshow(C)
